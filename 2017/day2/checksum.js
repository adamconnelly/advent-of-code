...
/**
 * Calculates a checksum on a spreadsheet of rows and cells.
 * @param {String} input - the input for the checksum. Cells are separates by tabs, rows are separated by newlines.
 */
 ...
 * @param {String} input - the input for the checksum. Cells are separates by tabs, rows are separated by newlines.
 */
module.exports.checksumV2 = (input) => {
    return input.split('\n')
        .map(line => calculateChecksumForLinePart2(line))
        .reduce((accumulator, value) => accumulator + value)
}

/**
 * The puzzle input for day 2.
 */
module.exports.PUZZLE_INPUT = `5048	177	5280	5058	4504	3805	5735	220	4362	1809	1521	230	772	1088	178	1794
...
function calculateChecksumForLine(line) {
    ...
    for (const cell of cells) {
        ...
        lowest = Math.min(lowest || cell, cell)
    }

    return highest - lowest
}

function calculateChecksumForLinePart2(line) {
    const cells = getCells(line)

    // Calculate all the possible combinations of values,
    // then remove everything other than the one combination
    // that evenly divides
    const sortedCells = cartesian([cells, cells])
        .filter(row => {
            return row[0] !== row[1] && row[0] % row[1] === 0
        })

    if (sortedCells.length > 0) {
        const row = sortedCells[0]
...
function getCells(line) {
    return line.trim().split(/[\t ]+/).map(cell => parseInt(cell))
}
