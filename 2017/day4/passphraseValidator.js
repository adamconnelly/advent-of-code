 ...
 * @param {Function} wordConverter a function that converts a word in the passphrase to a value that can be used for grouping to detect duplicates
 */
module.exports.isValid = function (passphrase, wordConverter) {
    if (passphrase) {
        wordConverter = wordConverter || (word => word)
        const wordCountMap = passphrase.split(' ')
            .reduce((words, currentWord) => {
                words[wordConverter(currentWord)] = (words[wordConverter(currentWord)] || 0) + 1
                return words
            }, {})

        const wordCounts = Object.keys(wordCountMap)
            .map(word => wordCountMap[word])

        return wordCounts.length > 1 && !wordCounts.some(count => count > 1)
    }
 ...
 */
module.exports.PUZZLE_INPUT = `bdwdjjo avricm cjbmj ran lmfsom ivsof
